...
#' This can be added to adjust for in the analysis for survival and continuous outcome variable types.
#' @param strat.var vector specifying the stratification variables. This can be added for the survival outcome variable type.
...
#' @param outcome.type outcome type of the 'outcome' variable. Can be either "continuous", "binary", or "survival".
...
#' @param xticks x-tick marks. Default is \code{seq(0.25, 0.75, 0.25)}.
...
PlotSTEPP <- function(data,
    ...
    if (is.null(covariate.var) & is.null(strat.var)) {
        effect.ac <- StatSummary(Outcome, 1:length(Biomarker), Treatment,
                               placebo.code, active.code, outcome.type,
                               alpha,
                               covariate.var = NULL,
