...
#' @param var Name of a variable (e.g. a biomarker variable). It can be a list of variables. Should be in colnames of \code{data}.  
...
#' @param main The main title. Default is \code{"Distribution of"}. A variable name will be added at the end of the title.
...
#' @param xlab X axis label. Default is \code{ifelse(log2 == TRUE, "(log2 scale)", "")}. A variable name will be added at the beginning of the label.
...
#' PlotDens(data=input, var=list("KRAS.exprs", "OS"), log2=TRUE,
#'          par.param=list(mar=c(4,4,3,2), mfrow=c(1,2), cex=0.75))
#' 
#' @export
...
PlotDens <- function(data,
                     ...
                     add.num=0,
                     xlab=ifelse(log2 == TRUE, "(log2 scale)", ""),
                     pdf.name=NULL,
    ...
    for (i in 1:length(var)) {
        ...
        hist(V, prob=T, main=paste(main, var[[i]], sep=" "), xlab=paste(var[[i]], xlab, sep=" "), col="grey", ylim=c(0, y2))
