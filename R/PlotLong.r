...
PlotLong <- function(data, mapping, formula = NULL, model = lm, model.args = NULL, 
    ...
    data %>% ggplot() + mapping + 
      ...
      (if (!is.null(facet.fun)) facet_grid(facet.fun) else facet_null() ) +
      (if (!is.null(formula)) ylab(paste("Adjusted", deparse(formula[[2]]))) else NULL) +
      ggwrap(xlab, 'xlab', list(...), null.empty = T) +
      ggwrap(ylab, 'ylab', list(...), null.empty = T) + 
      ggwrap(labs, 'labs',  list(...), null.empty = T)
